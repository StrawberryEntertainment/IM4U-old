...
・ MMDモデル及びVMDファイルの互換性及び読み込み時のデータ落ち修正
	⇒進捗率(30%)→互換性に関しては難しいと推測。
・ 特殊ボーンのWeightの対応
	⇒進捗率(0%)→本プラグインで作成したモデルの場合ウェイト設定がおかしい問題あり。
・ MMDモデルの剛体からPhATアセットの作成処理
	⇒進捗率(1%::標準アセットのみ生成)
・ VMDインポート処理(IKおよび物理挙動、ボーンのアニメーション)
	⇒進捗率(60%)->ただしIKに関しては殆ど出来そうにない…
...
　→現在対応しているボーンのWeight種別が1種類のみである為、見栄えが悪くなります。
　　デフォルトのアニメーション作成機能はまだ未完成です。
...
## ver 0.7.x 15/xx/xx リリース未定 予約

・VMDインポート機能の追加(IKの簡易再現:：VMDインポート時にアニメーションにベイクする形で事前計算で対応)
　ただし未完成及びバグ残存がある可能性あり。(PMX系のみ暫定対応、PMDに関しては未対応など)
・他、バグ修正、リファクタリング。？
