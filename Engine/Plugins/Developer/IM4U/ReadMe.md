...
→4.8以降ではソースの修正が一部必要となります。**ただし現時点ではサポート外のため動作確認はしていません**  
　修正箇所に関しては、PmxSkeltalMeshImport.cpp の FAsyncImportMorphTargetWork クラスにある  
　関数 GetStatId でコメントアウトしている箇所を解除し関数を有効にして下さい。コンパイルが可能なはずです。  
...
　同様に物理演算の挙動を再現出来ないため、例えばMMMやMMDでBridgeなどのツールで事前にVMD内にIkや物理挙動を焼き込んでおくことで擬似再現が可能。  
...
データ型に関しては、「MMDExtend.h」に実装していますので必要があれば確認して下さい。  
...
- ~~MMDモデルを直にインポートして作成したモデルにアニメーションを追加した場合、期待しない頂点の位置となる。  
　→現在対応しているボーンのWeight種別が1種類のみである為、見栄えが悪くなります。~~←に関してはVer.0.6.7にて修正。  
　　プラグインによるVMDインポート機能はまだ未完成です。  
- VMDのモーションを直にインポートした場合、例えばZ軸（上）でターンするモーションで  
　上半身と下半身の回転方向が物理的にありえない組み合わせでねじれて見える。  
　→クォータニオンの使い方が間違っていると思われますがまだ修正出来ていません。  
　　→暫定的な解決策としては、短い感覚でダミーのキーを打って貰えると回転方向を強制的制御出来るはずです。（未確認）  
- モデルインポート時に生成するマテリアルアセットを使った場合、一部の暗色系が透過扱いになってしまう。  
